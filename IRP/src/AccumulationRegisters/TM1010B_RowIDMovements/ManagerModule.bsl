...
Function CheckBalance(Ref, ItemList_InDocument, Records_InDocument, Records_Exists, RecordType, Unposting, AddInfo = Undefined) Export
	...
	If RowIDInfoServerReuse.GetUseRowIDRegister() Then
		Return CheckBalanceRegister(Ref, ItemList_InDocument, Records_InDocument, Records_Exists, RecordType, Unposting, AddInfo);
	Else
		Return CheckBalanceCatalog(Ref, ItemList_InDocument, Records_InDocument, Records_Exists, RecordType, Unposting, AddInfo);
	EndIf;
EndFunction
	
Function CheckBalanceCatalog(Ref, ItemList_InDocument, Records_InDocument, Records_Exists, RecordType, Unposting, AddInfo = Undefined)
	Query = New Query();
...
Function CheckBalanceRegister(Ref, ItemList_InDocument, Records_InDocument, Records_Exists, RecordType, Unposting, AddInfo = Undefined)
	Query = New Query();
	Query.TempTablesManager = PostingServer.PrepareRecordsTables(GetLockFieldNames(), "RowID", ItemList_InDocument,
		Records_InDocument, Records_Exists, Unposting, AddInfo);
	...
	|FROM
	|	ItemList AS ItemList
	|		INNER JOIN AccumulationRegister.TM1010B_RowIDMovements.Balance(, (RowID, Step, Basis) IN
	|			(SELECT
	|				ItemList.RowID,
	|				ItemList.Step,
	|				ItemList.Basis
	...
	ErrorParameters = New Structure();
	ErrorParameters.Insert("GroupColumns", "Item, ItemKey, RowID, Step, Basis, LackOfBalance");
	ErrorParameters.Insert("SumColumns", "Quantity");
	ErrorParameters.Insert("FilterColumns", "Item, ItemKey, RowID, Step, Basis, LackOfBalance");
	ErrorParameters.Insert("Operation", "RowID movements");
	ErrorParameters.Insert("RecordType", RecordType);
	PostingServer.ShowPostingErrorMessage(Query.Execute().Unload(), ErrorParameters, AddInfo);
