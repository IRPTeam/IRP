...
Procedure FillTransactions(Object, AddInfo = Undefined) Export
	...
	Query.Text =
	"SELECT
	|	PosCashBalances.PaymentType AS PaymentType,
	|	PosCashBalances.Account AS Account,
	|	PosCashBalances.AmountTurnover AS Amount,
	|	PosCashBalances.CommissionTurnover AS Commission,
	|	PosCashBalances.Account.Currency AS Currency
	|FROM
	|	AccumulationRegister.R3050T_PosCashBalances.Turnovers(BEGINOFPERIOD(&BegOfPeriod, DAY), ENDOFPERIOD(&EndOfPeriod, DAY), ,
	|		Company = &Company
	|	AND Branch = &Branch) AS PosCashBalances";
