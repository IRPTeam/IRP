...
//	|		AND BatchKeys.PreliminaryID = BatchKeysInfo.PreliminaryID
	...
	|	BatchKeys.PreliminaryQuantity AS PreliminaryQuantityBalance,
	|	BatchKeys.PreliminaryAmount AS PreliminaryAmountBalance,
	|	BatchKeys.PreliminaryTaxAmount AS PreliminaryTaxAmountBalance,
	|	CASE
	|		WHEN Batches.Ref IS NULL
	|		OR NOT BatchKeys.SalesInvoice.Date IS NULL
	|			THEN 0
...
//			TableOrdering.Columns.Add("PreliminaryQuantityBalance");
			...
			For Each FilteredRow In FilteredRows Do
				...
				If FilteredRow.QuantityBalance = 0 And FilteredRow.PreliminaryQuantityBalance = 0 Then
					Continue;
				EndIf;
				
				If FilteredRow.Company <> Row.Company Then
					Continue;
				EndIf;

				If Not ValueIsFilled(FilteredRow.Batch) Then
					Continue;
				EndIf;
...
//				NewRow.PreliminaryQuantityBalance = FilteredRow.PreliminaryQuantityBalance;
			...
			For Each Row_Batch In FilteredRows Do
				...
				ExpenseAmounts = New Structure();
				For Each Res In AmountResources() Do
					ExpenseAmounts.Insert(Res, CalculateExpenseAmount(ExpenseQuantity, Row_Batch, BatchQuantityResourceNameBalance, Res + "Balance"));
				EndDo;
				...
				If ExpenseQuantity <> 0 Or ExpenseAmounts.InvoiceAmount <> 0 Or ExpenseAmounts.PreliminaryAmount Then
					NewRow = Tables.DataForExpense.Add();
					NewRow.BatchKey  = Row.BatchKey;
					NewRow.Document  = Row.Document;
					NewRow.Company   = Row.Company;
	...
	TypeAmount = Metadata.DefinedTypes.typeAmount.Type;
	
	TableOfNewReceivedBatches.Columns.Add("Quantity", TypeQuantity);
	TableOfNewReceivedBatches.Columns.Add("PreliminaryQuantity", TypeQuantity);
	
	For Each Res In AmountResources() Do
		TableOfNewReceivedBatches.Columns.Add(Res, TypeAmount);
	EndDo;
			
	TableOfNewReceivedBatches.Columns.Add("QuantityBalance", TypeQuantity);
	TableOfNewReceivedBatches.Columns.Add("PreliminaryQuantityBalance", TypeQuantity);
	
	For Each Res In AmountResources() Do
		TableOfNewReceivedBatches.Columns.Add(Res + "Balance", TypeAmount);
	EndDo;
	...
	ElsIf TypeOf(Document) = Type("DocumentRef.SalesReturn") Or TypeOf(Document) = Type("DocumentRef.RetailReturnReceipt") Then
		...
		For Each Row In TableOfReturnedBatches Do
			...
			For Each Res In AmountResources() Do
				NewRowReceivedBatch[Res] = Row[Res];
			EndDo;
			...
			For Each Res In AmountResources() Do
				NewRowReceivedBatch[Res + "Balance"] = Row[Res];
			EndDo;						
		EndDo;
...
Procedure CalculateCompositeDocument(Rows, Tables, DataForReceipt, DataForExpense, TableOfNewReceivedBatches)
	For Each Row_Receipt In DataForReceipt Do
		...
		For Each Res In AmountResources() Do
			TotalExpenseAmounts.Insert(Res, DataForExpense.Total(Res));
		EndDo;
		...
		For Each Row_Expense In DataForExpense Do
			...
			If TypeOf(Row_Expense.Document) = Type("DocumentRef.Bundling") Then
				...
				For Each Res In AmountResources() Do
					If TotalExpenseAmounts[Res] <> 0 And Row_Expense[Res] <> 0 Then
						NewRowBundleAmountValues[Res] = Row_Expense[Res] / (TotalExpenseAmounts[Res] / 100);
					EndIf;
				EndDo;				
			Else
		...
		If TypeOf(Row_Receipt.Document) = Type("DocumentRef.Production") Then
			...
			EndIf;	
		EndIf; // Document type is Production
...
Procedure CalculateDecompositeDocument(Rows, Tables, DataForReceipt, DataForExpense, TableOfNewReceivedBatches)
	For Each Row_Receipt In DataForReceipt Do
		...
		For Each Row_Expense In DataForExpense Do
			...
			While QuerySelection.Next() Do
				For Each Res In AmountResources() Do
					NewRow[Res] = NewRow[Res] + (Row_Expense[Res] / 100 * QuerySelection[Res]);
				EndDo;
			EndDo;
		EndDo;
...
Function GetSalesBatches(SalesInvoice, DataForSalesBatches, BatchKey)
	...
	|	CASE
	|		WHEN R6050T_SalesBatchesTurnovers.PreliminaryQuantityTurnover <> 0
	|			THEN TRUE
	|		ELSE FALSE
	|	END AS IsPreliminary
	|INTO SalesBatches
	|FROM
