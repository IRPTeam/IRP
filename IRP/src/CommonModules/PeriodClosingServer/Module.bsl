...
#Region DepreciationCalculation

Function DepreciationCalculation(Company, StartDate, EndDate, ArrayOfBranches, Periodicity) Export
	PeriodsTable = GetPeriodsTable(StartDate, EndDate, Periodicity);
	
	AllErrors = New Array();
	For Each Branch In ArrayOfBranches Do
		Errors = DepreciationCalculation_ByBranch(Company, StartDate, EndDate, Branch, PeriodsTable);
		If TypeOf(Errors) = Type("Array") Then
...
Function DepreciationCalculation_ByBranch(Company, StartDate, EndDate, Branch, PeriodsTable);
	Query = New Query();
	Query.Text = 
	"SELECT
	|	Doc.Ref
	|FROM
	|	Document.DepreciationCalculation AS Doc
	|WHERE
	|	Doc.Posted
	|	AND Doc.Company = &Company
	...
	UnusedDocuments = GetUnusedDocuments("DepreciationCalculation");
	TotalCount = PeriodsTable.Count();
	...
	Msg = BackgroundJobAPIServer.NotifySettings();
	Msg.Log = "Start depreciation calculation: " + TotalCount;
	BackgroundJobAPIServer.NotifyStream(Msg);
	...
	For Each Period In PeriodsTable Do
		Try
			...
			If UnusedDocuments.Count() > 0 Then
				...
				UnusedDocuments.Delete(0);
			Else
				DocObject = Documents.DepreciationCalculation.CreateDocument();
			EndIf;
			
			DocObject.Date = EndOfDay(Period.EndDate);
			DocObject.DeletionMark = False;
			DocObject.Company = Company;
			DocObject.Branch = Branch;
			DocObject.Calculations.Clear();
			
			DocObject.Calculations.Load(
				Documents.DepreciationCalculation.GetCalculations(DocObject.Ref, 
					DocObject.Date, 
					DocObject.Company, 
					DocObject.Branch));
