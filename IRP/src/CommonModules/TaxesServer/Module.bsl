...
Function GetTaxesInfo(Date, Company, DocumentName, TransactionType, TaxKind = Undefined) Export
	//#@2094
	//Return ServerReuse.GetTaxesInfo(Date, Company, DocumentName, TransactionType, TaxKind);
...
Function _GetTaxesInfo(Date, Company, DocumentName, TransactionType, TaxKind = Undefined) Export
	...
	|	AND CASE
	|		WHEN &Filter_TaxKind
	|			THEN Tax.Kind = &TaxKind
	|		ELSE CASE
	|			WHEN &NotVAT
	|				THEN Tax.Kind <> VALUE(Enum.TaxKind.VAT)
	|			ELSE TRUE
	|		END
	|	END) AS TaxesSliceLast
	|WHERE
	|	TaxesSliceLast.Use
	|;
	|
	...
	//#@2094
	Query.SetParameter("Filter_TaxKind" , ValueIsFilled(TaxKind));
	Query.SetParameter("TaxKind"        , TaxKind);
	// temp
	If Upper(DocumentName) = Upper("SalesOrder") Then
		Query.SetParameter("NotVAT", True);	
	Else
		Query.SetParameter("NotVAT", False);
	EndIf;	
