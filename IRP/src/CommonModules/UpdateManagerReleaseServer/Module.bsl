...
Procedure GetUpdateInfo(ArrayOfUpdateInfo) Export
	...
	UpdateInfo = UpdateManagerServer.GetUpdateInfoDefenition();
	UpdateInfo.Method = "UpdateManagerReleaseServer.RunUpdate_DBMigration_IRP680";
	UpdateInfo.Description = R().Update_005;
	UpdateInfo.FullDescription = R().UpdateDesc_005;
	ArrayOfUpdateInfo.Add(UpdateInfo);				
EndProcedure

Function RunUpdate_DBMigration_IRP680(MethodName) Export	
	Query = New Query();
	Query.Text = 
	"SELECT
	|	Doc.Ref
	|FROM
	|	Document.Payroll AS Doc
	|WHERE
	...
	Msg.Log = "Start update Payrolls: " + TotalCount;
	...
	While QuerySelection.Next() Do
		Try
			...
			For Each Row In Obj.SalaryTaxList Do
				Row.Partner = Obj.DELETE_Partner;
				Row.LegalName = Obj.DELETE_LegalName;
			EndDo;
			Obj.Write(?(Obj.Posted, DocumentWriteMode.Posting, DocumentWriteMode.Write));
		Except
