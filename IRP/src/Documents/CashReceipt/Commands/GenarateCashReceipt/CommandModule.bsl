...
Function ErrorMessageStructure(BasisDocuments)
	...
	For Each BasisDocument In BasisDocuments Do
		...
		If ValueIsFilled(ErrorMessageKey) Then
			ErrorMessageStructure.Insert(ErrorMessageKey, StrTemplate(R()[ErrorMessageKey], Metadata.Documents.CashReceipt.Synonym));
		EndIf;
	EndDo;
	...
	Else
		ErrorMessageText = StrTemplate(R().Error_059, Metadata.Documents.CashReceipt.Synonym) + Chars.LF +
...
Function JoinDocumentsStructure(ArrayOfTables)
	...
	ValueTableCopy.GroupBy("BasedOn, TransactionType, Company, CashAccount, Currency, CurrencyExchange");
