...
Function PostingGetDocumentDataTables(Ref, Cancel, PostingMode, Parameters, AddInfo = Undefined) Export
	...
	AccMetadata = Metadata.AccumulationRegisters.R5012B_VendorsAging;
	...
	For Each Row In Ref.Transactions Do
		...
		ElsIf DocMetadata.TabularSections.Find("VendorsPaymentTerms") <> Undefined Then
			Query.Text = 
			"SELECT
			|	PaymentTerms.Ref AS DocRef,
			|	MAX(PaymentTerms.Date) AS PaymentDate
			|FROM
			|	Document.OpeningEntry.VendorsPaymentTerms AS PaymentTerms
			|		LEFT JOIN Document.OpeningEntry.AccountPayableByDocuments AS AR
