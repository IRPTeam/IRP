...
Function Calculations()
	Return
		...
		|INTO Calculations
		|FROM
		|	Document.FixedAssetRevaluation.Calculations AS Calculations
		|WHERE
		|	Calculations.Ref = &Ref AND Calculations.Amount <> 0";
EndFunction	
...
Function R8515T_CostOfFixedAsset()
	Return
		...
		|	Calculations.FixedAsset,
		|	Calculations.LedgerType,
		|	Calculations.NewAmountBalance - Calculations.InitialAmount AS Amount,
		|	&Date AS Period,
		|	Calculations.Ref.Company.LandedCostCurrencyMovementType.Currency AS Currency
		...
		|	Calculations
		|WHERE
		|	(Calculations.NewAmountBalance - Calculations.InitialAmount) <> 0";
EndFunction
...
Function GetCalculations(Ref, Date, Company, Branch, FixedAsset = Undefined) Export
	...
	|	ActiveFixedAssets.AmountBalance AS NewAmountBalance,
	|	CostFixedAsset.AmountTurnover AS InitialAmount,
...
Function T1040T_AccountingAmounts()
	Return 
		...
		|	VALUE(Catalog.AccountingOperations.FixedAssetRevaluation_DR_R8510B_BookValueOfFixedAsset_CR_R5021T_Revenues) AS Operation,
...
Function GetAccountingAnalytics(Parameters) Export
	...
	If Parameters.Operation = Operations.FixedAssetRevaluation_DR_R8510B_BookValueOfFixedAsset_CR_R5021T_Revenues Then
		Return GetAnalytics_Depreciation_Revenues(Parameters); // Depreciation - Revenues
	ElsIf Parameters.Operation = Operations.FixedAssetRevaluation_DR_R5022T_Expenses_CR_R8510B_BookValueOfFixedAsset Then
...
Function GetAnalytics_Expenses_Depreciation(Parameters)
	...
	AdditionalAnalyticsValues = New Structure("ExpenseType", Parameters.RowData.ExpenseType);
	AccountingServer.SetDebitExtDimensions(Parameters, AccountingAnalytics, AdditionalAnalyticsValues);
...
Function GetAnalytics_Depreciation_Revenues(Parameters)
	AccountingAnalytics = AccountingServer.GetAccountingAnalyticsResult(Parameters);
	AccountParameters   = AccountingServer.GetAccountParameters(Parameters);
	...
	AdditionalAnalyticsValues = New Structure("RevenueType", Parameters.RowData.RevenueType);
	AccountingServer.SetCreditExtDimensions(Parameters, AccountingAnalytics, AdditionalAnalyticsValues);
	Return AccountingAnalytics;
EndFunction
