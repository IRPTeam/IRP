...
Procedure CommandProcessing(CommandParameter, CommandExecuteParameters)
	...
	For Each Basis In CommandParameter Do
		If TypeOf(Basis) = Type("DocumentRef.SalesReportFromTradeAgent") Then
			ArrayOf_SalesReportFromTradeAgent.Add(Basis);
		Else
	...
	Else
		...
		For Each FillingData In DocumentStructure Do
			OpenForm("Document.PurchaseInvoice.ObjectForm", New Structure("FillingValues", FillingData), , New UUID());
		EndDo;		
	EndIf;
EndProcedure
...
Function JoinDocumentsStructure(ArrayOfTables)
	...
	DocMetadata = Metadata.Documents.PurchaseInvoice.Attributes;
	...
	ItemListMetadata = Metadata.Documents.PurchaseInvoice.TabularSections.ItemList.Attributes;
	...
	ValueTable.Columns.Add("ExpenseType"      , ItemListMetadata.ExpenseType.Type);
	...
	For Each Row In ValueTableCopy Do
		...
		For Each RowItemList In ItemList Do
			...
			NewRow.Insert("ExpenseType"      , RowItemList.ExpenseType);
...
Function GetDocumentTable_SalesReportToConsignor(ArrayOfBasisDocuments)
	...
	Query.Text =
	"SELECT
	|	""SalesReportFromTradeAgent"" AS BasedOn,
	|	ItemList.Ref.Company AS Company,
	|	VALUE(Enum.PurchaseTransactionTypes.Purchase) AS TransactionType,
	...
	|	ItemList.Ref.Agreement.TradeAgentFeeExpenseRevenueType AS ExpenseType
	|FROM
	...
	|	VALUE(Enum.PurchaseTransactionTypes.Purchase)";
