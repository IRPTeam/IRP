...
Function GetArrayOfUnusedFiles(PathForSave)
...
Procedure DeleteUnusedFiles(PathForSave, ArrayOfFilesID)
...
Function FileTransferPOST(Request)
	...
	If GetUnusedFiles <> Undefined Then
		Try
			Return CreateSuccessResponse(HttpResponse,
				New Structure("ArrayOfUnusedFiles", GetArrayOfUnusedFiles(FileStorageInfo.PathForSave)));
		Except
	...
	If DeleteUnusedFiles <> Undefined Then
		Try
			DeleteUnusedFiles(FileStorageInfo.PathForSave
