...
Scenario: _022323 check movements of the document Purchase return order in the PurchaseTurnovers (store does not use Shipment confirmation, without Purchase return order) - minus
	Given I open hyperlink "e1cib/list/AccumulationRegister.PurchaseTurnovers"
	And "List" table contains lines
...
Scenario: _022336 check the output of the document movement report for Purchase Return
	...
	* Check the report generation
		...
		| ''                                    | 'Receipt'     | '*'      | '450,2'     | 'Main Company' | ''               | 'Ferron BP' | 'Company Ferron BP' | 'Vendor Ferron, USD' | 'TRY'      | 'Local currency'               | 'No'                   |
		...
		| ''                                      | '*'      | '-2'        | '-450,2' | 'Main Company' | '$$PurchaseInvoice018006$$' | 'TRY'      | 'L/Green'  | '*'       | 'Local currency'               | 'No'                   | '' |
