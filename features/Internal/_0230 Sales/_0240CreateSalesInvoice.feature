...
Scenario: _024001 create document Sales Invoice based on sales order (partial quantity, SO-SI)
	...
	* Check auto filling inventory origin (FO Use commission trading switched off)
		...
		And "ItemList" table contains lines
			| '#' | 'Inventory origin' | 'Sales person' | 'Price type'        | 'Item'  | 'Item key' | 'Profit loss center'      | 'Dont calculate row' | 'Tax amount' | 'Unit'           | 'Serial lot numbers' | 'Quantity' | 'Price'    | 'VAT' | 'Offers amount' | 'Net amount' | 'Total amount' | 'Use work sheet' | 'Other revenue type' | 'Additional analytic' | 'Store'    | 'Delivery date' | 'Use shipment confirmation' | 'Detail' | 'Sales order'                             | 'Work order' | 'Revenue type' |
...
Scenario: _024028 create SI based on SC with two same items (creation based on)
	...
	* Check
		And "ItemList" table contains lines
			| '#'   | 'Price type'          | 'Item'    | 'Item key'   | 'Profit loss center'        | 'Dont calculate row'   | 'Tax amount'   | 'Serial lot numbers'   | 'Quantity'   | 'Unit'   | 'Price'    | 'VAT'   | 'Offers amount'   | 'Net amount'   | 'Total amount'   | 'Other revenue type'   | 'Additional analytic'   | 'Store'      | 'Delivery date'   | 'Use shipment confirmation'   | 'Detail'   | 'Sales order'                                   | 'Revenue type'    |
