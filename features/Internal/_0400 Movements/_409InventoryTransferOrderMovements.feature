...
Scenario: _040359 check Inventory transfer order movements by the Register  "Posted documents registry"
	And I close all client application windows
	Given I open hyperlink "e1cib/list/Document.InventoryTransferOrder"
	And I go to line in "List" table
		| 'Number' |
	...
	* Check movements by the Register "Posted documents registry"
		...
		Then "ResultTable" spreadsheet document is equal
			...
			| ''                                                      | 'Inventory transfer order 21 dated 16.02.2021 16:14:02' | '16.02.2021 16:14:02' | '21'     | '*'           | '*'           | 'en description is empty' | 'en description is empty' | 'No'                    |
	And I close all client application windows		
