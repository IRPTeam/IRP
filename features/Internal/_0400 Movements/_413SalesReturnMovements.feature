...
Scenario: _041340 check Sales return movements by the Register  "R8012 Consignor inventory" (Return Consignor stocks)
	...
	* Check movements by the Register  "R8012 Consignor inventory"
		...
		Then "ResultTable" spreadsheet document is equal
			...
			| ''                                           | 'Record type' | 'Period'              | 'Resources' | 'Dimensions'   | ''         | ''            | ''                         |
			| ''                                           | ''            | ''                    | 'Quantity'  | 'Company'      | 'Item key' | 'Partner'     | 'Agreement'                |
			| ''                                           | 'Receipt'     | '05.11.2022 00:00:00' | '1'         | 'Main Company' | 'UNIQ'     | 'Consignor 1' | 'Consignor partner term 1' |
			| ''                                           | 'Receipt'     | '05.11.2022 00:00:00' | '2'         | 'Main Company' | 'S/Yellow' | 'Consignor 1' | 'Consignor partner term 1' |		
		And I close all client application windows	

Scenario: _041341 check Sales return movements by the Register  "R8013 Consignor batch wise ballance" (Return Consignor stocks)
	And I close all client application windows
	* Select Sales return
		Given I open hyperlink "e1cib/list/Document.SalesReturn"
		And I go to line in "List" table
	...
	* Check movements by the Register  "R8013 Consignor batch wise ballance"
		...
		Then "ResultTable" spreadsheet document is equal
			...
			| ''                                                | 'Receipt'     | '05.11.2022 00:00:00' | '1'         | 'Main Company' | 'Purchase invoice 195 dated 02.11.2022 16:31:38' | 'Store 02' | 'UNIQ'     |
