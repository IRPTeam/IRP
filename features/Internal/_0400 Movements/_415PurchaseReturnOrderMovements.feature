...
Scenario: _041503 check Purchase return order movements by the Register  "Posted documents registry"
	And I close all client application windows
	Given I open hyperlink "e1cib/list/Document.PurchaseReturnOrder"
	And I go to line in "List" table
		| 'Number' |
	...
	* Check movements by the Register "Posted documents registry"
		...
		Then "ResultTable" spreadsheet document is equal
			...
			| ''                                                    | 'Purchase return order 231 dated 14.03.2021 18:52:33' | '14.03.2021 18:52:33' | '231'    | '*'           | '*'           | 'en description is empty' | 'en description is empty' | 'No'                    |
		And I close all client application windows		
