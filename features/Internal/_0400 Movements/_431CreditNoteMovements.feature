...
Scenario: _043014 check Credit note movements by the Register  "T2015 Transactions info" (with vendor)
	...
	* Check movements by the Register  "T2015 Transactions info" 
		...
		Then "ResultTable" spreadsheet document is equal
			...
			| ''                                        | 'Main Company' | 'Front office' | ''      | '05.04.2021 09:30:47' | '*'   | 'TRY'      | 'Ferron BP' | 'Company Ferron BP' | 'Vendor Ferron, TRY' | 'Yes'                   | 'No'                      | 'Credit note 1 dated 05.04.2021 09:30:47' | '*'         | ''        | 'en description is empty' | 'TRY'                  | '500'    | 'Yes'    | 'No'      |
	And I close all client application windows
...
Scenario: _043016 check Credit note movements by the Register  "T2015 Transactions info" (with customer)
	...
	* Check movements by the Register  "T2015 Transactions info" 
		...
		Then "ResultTable" spreadsheet document is equal
			...
			| 'Register  "T2015 Transactions info"'     | ''             | ''             | ''      | ''                    | ''    | ''         | ''          | ''                  | ''                         | ''                      | ''                        | ''                                        | ''          | ''        | ''                        | ''                     | ''       | ''       | ''        |
			| ''                                        | 'Company'      | 'Branch'       | 'Order' | 'Date'                | 'Key' | 'Currency' | 'Partner'   | 'Legal name'        | 'Agreement'                | 'Is vendor transaction' | 'Is customer transaction' | 'Transaction basis'                       | 'Unique ID' | 'Project' | 'Currency movement type'  | 'Transaction currency' | 'Amount' | 'Is due' | 'Is paid' |
			| ''                                        | 'Main Company' | 'Front office' | ''      | '05.04.2021 09:30:58' | '*'   | 'TRY'      | 'Ferron BP' | 'Company Ferron BP' | 'Basic Partner terms, TRY' | 'No'                    | 'Yes'                     | 'Credit note 2 dated 05.04.2021 09:30:58' | '*'         | ''        | 'en description is empty' | 'TRY'                  | '-700'   | 'Yes'    | 'No'      |	
	And I close all client application windows
