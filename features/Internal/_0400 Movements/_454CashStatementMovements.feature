...
Scenario: _045403 check CashStatement movements by the Register  "R3021 Cash in transit (incoming)"
	...
	* Check movements by the Register  "R3021 Cash in transit (incoming)"
		...
		Then "ResultTable" spreadsheet document is equal
			...
			| ''                                             | 'Receipt'     | '23.06.2022 22:53:32' | '34,24'     | '3,42'       | 'Main Company' | 'Distribution department' | 'POS account, Comission separate, TRY' | 'Reporting currency'           | 'USD'      | 'TRY'                  | 'Bank account, TRY'  | ''      | 'No'                   |
			| ''                                             | 'Receipt'     | '23.06.2022 22:53:32' | '200'       | '20'         | 'Main Company' | 'Distribution department' | 'POS account, Comission separate, TRY' | 'Local currency'               | 'TRY'      | 'TRY'                  | 'Bank account, TRY'  | ''      | 'No'                   |
			| ''                                             | 'Receipt'     | '23.06.2022 22:53:32' | '200'       | '20'         | 'Main Company' | 'Distribution department' | 'POS account, Comission separate, TRY' | 'en description is empty'      | 'TRY'      | 'TRY'                  | 'Bank account, TRY'  | ''      | 'No'                   |
		And I close all client application windows
