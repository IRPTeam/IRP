...
Scenario: _047004 check Sales report to consignor movements by the Register  "T2015 Transactions info"
	...
	* Check movements by the Register  "T2015 Transactions info"
		...
		Then "ResultTable" spreadsheet document is equal
			...
			| ''                                                       | 'Amount'    | 'Is due' | 'Is paid' | 'Company'      | 'Branch'                  | 'Date'                | 'Key'                                  | 'Currency' | 'Partner'     | 'Legal name'  | 'Agreement'                | 'Order' | 'Is vendor transaction' | 'Is customer transaction' | 'Transaction basis' | 'Unique ID' |
