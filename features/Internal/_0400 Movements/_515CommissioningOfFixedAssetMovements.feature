...
Scenario: _051505 check Commissioning of fixed asset movements by the Register "R6020 Batch balance"
	...
	* Check movements by the Register  "R6020 Batch balance" 
		...
		Then "ResultTable" spreadsheet document is equal
			...
			| ''                                                          | 'Period'              | 'RecordType' | 'Company'      | 'Store'    | 'Item key'      | 'Batch'                                         | 'Batch key'                | 'Inventory origin' | 'Source of origin' | 'Serial lot number' | 'Partner' | 'Agreement' | 'Legal name' | 'Quantity' | 'Invoice amount' | 'Invoice tax amount' | 'Indirect cost amount' | 'Indirect cost tax amount' | 'Extra cost amount by ratio' | 'Extra cost tax amount by ratio' | 'Extra direct cost amount' | 'Extra direct cost tax amount' | 'Allocated cost amount' | 'Allocated cost tax amount' | 'Allocated revenue amount' | 'Allocated revenue tax amount' | 'Total amount' | 'Total net amount' | 'Total tax amount' | 'Preliminary quantity' | 'Preliminary amount' | 'Preliminary tax amount' | 'Calculation movement cost'                               |
...
Scenario: _051508 check Commissioning of fixed asset movements by the Register "T6020 Batch keys info"
	...
	* Check movements by the Register  "T6020 Batch keys info" 
		...
		Then "ResultTable" spreadsheet document is equal
			...
			| ''                                                          | '12.01.2024 12:00:00' | 'Main Company' | 'Front office' | 'Store 02' | 'Fixed asset 1' | 'Expense'   | ''                       | ''         | ''               | ''              | '                                    ' | 'Logistics department' | ''             | ''     | ''           | ''                       | ''                  | ''                 | ''                    | ''                          | 'Computer Servers' | 'No'             | ''               | '1'        | ''               | ''                   | ''                     | ''                         | ''                           | ''                               | ''                         | ''                             | ''                      | ''                          | ''                         | ''                             | ''                             | ''                             | ''                             |		
	And I close all client application windows
