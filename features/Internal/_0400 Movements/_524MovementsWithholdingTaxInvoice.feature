...
Scenario: _052422 check With holding Tax Invoice movements by register "T2015 Transactions info" (transaction currency not equal local currency)
	...
	* Check movements
		...
		Then "ResultTable" spreadsheet document is equal
			...
			| 'Register  "T2015 Transactions info"'                 | ''             | ''       | ''      | ''                    | ''                                     | ''         | ''          | ''                  | ''                   | ''                      | ''                        | ''                                                    | ''          | ''        | ''                        | ''                     | ''       | ''       | ''        |
			| ''                                                    | 'Company'      | 'Branch' | 'Order' | 'Date'                | 'Key'                                  | 'Currency' | 'Partner'   | 'Legal name'        | 'Agreement'          | 'Is vendor transaction' | 'Is customer transaction' | 'Transaction basis'                                   | 'Unique ID' | 'Project' | 'Currency movement type'  | 'Transaction currency' | 'Amount' | 'Is due' | 'Is paid' |
			| ''                                                    | 'Main Company' | ''       | ''      | '25.09.2025 18:12:10' | '                                    ' | 'USD'      | 'Ferron BP' | 'Company Ferron BP' | 'Vendor Ferron, TRY' | 'Yes'                   | 'No'                      | 'Withholding tax invoice 6 dated 25.09.2025 18:12:10' | '*'         | ''        | 'en description is empty' | 'TRY'                  | '150'    | 'Yes'    | 'No'      |
	And I close all client application windows		
