...
Feature: create Bank payment simple form
...
Scenario: _175000 preparation (Bank payment simple form)
...
Scenario: _1750001 check preparation
	When check preparation	

Scenario: _175001 create Bank payment based on Purchase invoice - Payment to the vendor (simple form)
	...
	* Check filling BP
		And form attributes have values:
			...
			| 'PaymentListAgreementNoSplits'     | "Partner term with vendor (advance payment by document)" | '' |
	...
	* Reselect basis document
		...
		And I go to line in "List" table
			| "Amount"   | "Document"                                     |
			| "4 415,00" | "Purchase invoice 5 dated 02.12.2023 12:30:00" |
		And I select current line in "List" table
		Then the form attribute named "PaymentListBasisDocumentNoSplits" became equal to "Purchase invoice 5 dated 02.12.2023 12:30:00"	
	* Change in payment amount
		...
		And I click Select button of "Financial movement type" field
		And I go to line in "List" table
			| "Description"       |
			| "Payment to vendor" |
		And I select current line in "List" table	
...
Scenario: _175002 create Bank payment based on Sales return - Return to customer (simple form)
		And I close all client application windows
	* Select Sales return
		Given I open hyperlink "e1cib/list/Document.SalesReturn"
		And I go to line in "List" table
	...
	* Reselect basis document
		...
		And I go to line in "List" table
			| "Amount" | "Document"                                 |
			| "270,00" | "Sales return 6 dated 21.08.2023 12:00:00" |
		And I select current line in "List" table
		Then the form attribute named "PaymentListBasisDocumentNoSplits" became equal to "Sales return 6 dated 21.08.2023 12:00:00"	
	* Change in payment amount
		And I input "120,00" text in the field named "PaymentListTotalAmountNoSplits"
		And I click Choice button of the field named "PaymentListProjectNoSplits"
		And I go to line in "List" table
		...
		Then the form attribute named "PaymentListProjectNoSplits" became equal to "Project 1"
		Then the form attribute named "PaymentListCashFlowCenterNoSplits" became equal to "Business unit 1"
		And I select from the drop-down list named "PaymentListFinancialMovementTypeNoSplits" by "Refund to customer" string		
	* Post document and check saving
		And I click the button named "FormPost"
		And I delete "$$NumberBankPayment2$$" variable
		And I delete "$$BankPayment2$$" variable
		And I save the value of "Number" field as "$$NumberBankPayment2$$"
		And I save the window as "$$BankPayment2$$"
		And I click the button named "FormPostAndClose"
		Given I open hyperlink "e1cib/list/Document.BankPayment"		
		And "List" table contains lines
			| 'Number'                 |
...
Scenario: _1750012 create Bank payment - Salary payment (simple form)
	And I close all client application windows
	* create Bank payment
		Given I open hyperlink "e1cib/list/Document.BankPayment"
	...
	* Filling main details
		...
		And I input"Bank account, TRY" text in "Account" field
	...
	* Check
		...
		Then the form attribute named "TransactionType" became equal to "Salary payment"			
	* Post document and check saving
		And I click the button named "FormPost"
		And I delete "$$NumberBankPayment4$$" variable
		And I delete "$$BankPayment4$$" variable
		And I save the value of "Number" field as "$$NumberBankPayment4$$"
		And I save the window as "$$BankPayment4$$"
		And I click the button named "FormPostAndClose"
		Given I open hyperlink "e1cib/list/Document.BankPayment"		
		And "List" table contains lines
			| 'Number'                 |
...
Scenario: _1750013 create Bank payment - Other partner (simple form)
	And I close all client application windows
	* create Bank payment 
		Given I open hyperlink "e1cib/list/Document.BankPayment"
		And I click the button named "FormCreate"
	...
	* Filling main details
		...
		And I select from the drop-down list named "PaymentListRevenueTypeNoSplits" by "Other revenues" string
		And I select from the drop-down list named "PaymentListProfitLossCenterNoSplits" by "Business unit 1" string
		And I click Choice button of the field named "PaymentListTaxNoSplits"
		And I go to line in "List" table
			| "Description"          |
			| "Income Tax (Emloyee)" |
		And I select current line in "List" table
		And I input "10,00" text in the field named "PaymentListTaxDiscountAmountNoSplits"	
	* Check
		Then the form attribute named "Branch" became equal to "Business unit 1"
		Then the form attribute named "Account" became equal to "Bank account, TRY"
		Then the form attribute named "Company" became equal to "Own company 2"
		...
		Then the form attribute named "PaymentListRevenueTypeNoSplits" became equal to "Other revenues"
		Then the form attribute named "PaymentListProfitLossCenterNoSplits" became equal to "Business unit 1"
		Then the form attribute named "PaymentListTaxNoSplits" became equal to "Income Tax (Emloyee)"
		And the editing text of form attribute named "PaymentListTaxDiscountAmountNoSplits" became equal to "10,00"
		And the editing text of form attribute named "PaymentListTotalAmountNoSplits" became equal to "100,00"										
	* Post document and check saving
		And I click the button named "FormPost"
		And I delete "$$NumberBankPayment5$$" variable
		And I delete "$$BankPayment5$$" variable
		And I save the value of "Number" field as "$$NumberBankPayment5$$"
		And I save the window as "$$BankPayment5$$"
		And I click the button named "FormPostAndClose"
		Given I open hyperlink "e1cib/list/Document.BankPayment"		
		And "List" table contains lines
			| 'Number'                 |
...
Scenario: _175014 create Bank payment based on Employee cash advance - Employee cash advance (simple form)
		And I close all client application windows		
	...
	* Check filling CR
		...
		And form attributes have values:
			...
			| 'DetailsByRow'                      | "Yes"                                               | '' |
			| 'DetailsByRowNoSplits'              | "Yes"                                               | '' |
			| 'PaymentListBasisDocumentNoSplits'  | "Employee cash advance 3 dated 22.03.2024 11:52:17" | '' |
			| 'PaymentListCashFlowCenterNoSplits' | "Business unit 1"                                   | '' |
			| 'PaymentListPartnerNoSplits'        | "Employee 1"                                        | '' |
			| 'TransactionType'                   | "Employee cash advance"                             | '' |
		And the editing text of form attribute named "PaymentListTotalAmountNoSplits" became equal to "180,00"
	...
	* Post document and check saving
		And I click the button named "FormPost"
		And I delete "$$NumberBankPayment6$$" variable
		And I delete "$$BankPayment6$$" variable
		And I save the value of "Number" field as "$$NumberBankPayment6$$"
		And I save the window as "$$BankPayment6$$"
		And I click the button named "FormPostAndClose"
		Given I open hyperlink "e1cib/list/Document.BankPayment"
		And "List" table contains lines
			| 'Number'                 |
...
Scenario: _175025 Prevent negative refund transactions in Cash payment (simple form)
	And I close all client application windows
	* Open BP
		Given I open hyperlink "e1cib/list/Document.BankPayment"
		And I click the button named "FormCreate"
	...
	* Check
		...
		Then there are lines in TestClient message log
			|'Lack of advances [Customer 1 (3 partner terms)] [Partner term with customer (by document + credit limit)] [1 000 000]'|
		And I close all client application windows			