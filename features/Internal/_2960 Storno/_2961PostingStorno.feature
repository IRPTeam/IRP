...
Feature: create document Storno

As an accountant
I want to create a Storno document based on a posted document
To reverse the register movements of the basis document without unposting it
...
Scenario: _296100 preparation (Storno on Sales invoice from TestDB)
...
Scenario: _2961001 check preparation
	When check preparation


Scenario: _296101 create Storno based on posted Sales invoice using Storno command
	And I close all client application windows
	* Open the basis Sales invoice and run the Storno command
		Given I open hyperlink "e1cib/list/Document.SalesInvoice"
		And I go to line in "List" table
			| 'Number'  |
			| '1'       |
	...
	* Check the storno header is filled from the basis
		...
		Then the form attribute named "Basis" became equal to "Sales invoice 1 dated 24.02.2023 10:14:47"
	* Post the storno and remember its number
		And I click the button named "FormPost"
		And I wait "Number" field will be filled in "30" seconds
		Then system warning window does not appear
		And I delete "$$NumberStorno296101$$" variable
		And I save the value of "Number" field as "$$NumberStorno296101$$"
		And I click the button named "FormPostAndClose"
	* Check the storno was created and posted
...
Scenario: _296102 UndoPosting and repost the Storno keep it consistent
	And I close all client application windows
	* Open the created storno
		Given I open hyperlink "e1cib/list/Document.Storno"
		And I go to line in "List" table
			| 'Number'                   |
			| '$$NumberStorno296101$$'   |
...
Scenario: _296103 copy Storno preserves Company and Basis
	And I close all client application windows
	* Copy the created storno from the list
		Given I open hyperlink "e1cib/list/Document.Storno"
		And I go to line in "List" table
			| 'Number'                   |
			| '$$NumberStorno296101$$'   |
		And in the table "List" I click the button named "ListContextMenuCopy"
	* Check the copy inherited the header
		Then the form attribute named "Company" became equal to "Own company 2"
