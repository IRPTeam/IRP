...
Feature: check Storno movements

As an accountant
I want the Storno document to write reversing (negative) register movements of its basis
So that the basis document influence on the registers is cancelled without unposting it
...
Scenario: _296200 preparation (Storno movements)
...
And I close all client application windows
	...
	* Create and post a Storno on the posted Sales invoice 1
		And I close all client application windows
		Given I open hyperlink "e1cib/list/Document.SalesInvoice"
		And I go to line in "List" table
			| 'Number'  |
		...
		And I delete "$$NumberStornoMovements$$" variable
		And I save the value of "Number" field as "$$NumberStornoMovements$$"
		And I click the button named "FormPostAndClose"
		And I close all client application windows


Scenario: _2962001 check preparation
	When check preparation


# DRIVER SKELETON — capture the exact reversed-movement table on the first green run and
# replace the "report is generated" guard with a strict "ResultTable spreadsheet document is equal" table.
# Expected on capture: every resource is the basis value with the opposite sign; Period = Storno date.
Scenario: _296201 check Storno reverses movements by Register "R4010 Actual stocks"
	And I close all client application windows
	* Open the posted Storno
		Given I open hyperlink "e1cib/list/Document.Storno"
		And I go to line in "List" table
			| 'Number'                       |
			| '$$NumberStornoMovements$$'    |
	* Generate registrations report for the register
		And I click "Registrations report" button
		And I select "R4010 Actual stocks" exact value from "Register" drop-down list
		And I click "Generate report" button
		And I wait for filling of "r1c1" cell in "ResultTable" spreadsheet document for "30" seconds
	And I close all client application windows


Scenario: _296202 check Storno reverses movements by Register "R2001 Sales"
	And I close all client application windows
	* Open the posted Storno
		Given I open hyperlink "e1cib/list/Document.Storno"
		And I go to line in "List" table
...
Scenario: _296203 check Storno reverses movements by Register "R5010 Reconciliation statement"
	And I close all client application windows
	* Open the posted Storno
		Given I open hyperlink "e1cib/list/Document.Storno"
		And I go to line in "List" table
...
# NEGATIVE CHECK (Risk 4): basis information registers must NOT be reversed by Storno.
# On capture: confirm the storno produces NO records for the basis info registers
# (e.g. T2015S Transactions info, T6020S Batch keys info) via the report register drop-down.
Scenario: _296210 check Storno does NOT reverse information registers of the basis
	And I close all client application windows
	Given I open hyperlink "e1cib/list/Document.Storno"
	And I go to line in "List" table
		| 'Number'                       |
