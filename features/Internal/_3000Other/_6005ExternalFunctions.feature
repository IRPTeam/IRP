...
Scenario: _602711 check pause
	...
	* Sheduller settings
		...
		And "List" table contains lines
			| 'Enable' | 'Safe mode' | 'Use scheduler' | 'Type'    | 'Reference'         | 'Description'  | 'Job add time' | 'Job start time' | 'Job status' | 'Job finish time' |
			| 'Yes'    | 'Yes'       | 'Yes'           | 'Execute' | 'Pause 100'         | 'Pause 100'    | '*'            | ''               | 'Wait'       | ''                |		
		And Delay 150
		And "List" table contains lines
		| 'Enable' | 'Safe mode' | 'Use scheduler' | 'Type'    | 'Reference'         | 'Description'  | 'Job add time' | 'Job start time' | 'Job status' | 'Job finish time' |
		| 'Yes'    | 'Yes'       | 'Yes'           | 'Execute' | 'Pause 100'         | 'Pause 100'    | '*'            | '*'              | 'Active'     | ''                |		
				

Scenario: _602712 check button pause, continue, stop
	And I close all client application windows
	* Open External functions
		Given I open hyperlink "e1cib/list/Catalog.ExternalFunctions"
	* Select function
