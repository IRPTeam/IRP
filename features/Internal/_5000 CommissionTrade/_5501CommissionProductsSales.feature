...
Scenario: _050010 create SI (commission products sales)
	...
	* Post SI
		...
		And "ItemList" table became equal
			| '#' | 'Inventory origin' | 'Price type'              | 'Item'                                                                     | 'Item key' | 'Dont calculate row' | 'Tax amount' | 'Unit' | 'Serial lot numbers' | 'Source of origins' | 'Quantity' | 'Price'  | 'VAT' | 'Net amount' | 'Total amount' | 'Use work sheet' | 'Other revenue type'   | 'Store'    | 'Use shipment confirmation' |
