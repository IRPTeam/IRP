...
Scenario:_891300 preparation (stock inventory control)
	...
	* Delete control for register R4014B_SerialLotNumber
		...
		And I go to line in "List" table
			...
			| "CheckBalance_R4014B_SerialLotNumber" | "Custom"            | ""                | "Yes"   |
...
Scenario:_8913001 check preparation (stock inventory control)
	When check preparation 


Scenario:_8913002 check stock inventory control for RetailSalesReceipt
...
Scenario:_8913003 check stock inventory control for SI
	And I close all client application windows
...
Scenario:_8913004 check stock inventory control for PR
	And I close all client application windows
	And Delay 15
...
Scenario:_8913005 check stock inventory control for Stock adjustment as write off
	And I close all client application windows
...
Scenario:_8913006 check stock inventory control for IT
	And I close all client application windows
...
Scenario:_8913010 try unpost PI that make a plus on the store
	And I close all client application windows
	* Create PI
...
Scenario:_8913011 try unpost RetailReturnReceipt that make a plus on the store
	And I close all client application windows
	* Preparation
...
Scenario:_8913012 try unpost SalesReturn that make a plus on the store
	And I close all client application windows
	And Delay 80
	* Preparation
		* Remove control (store 05)
...
Scenario:_8913013 try unpost StockAdjustmentAsSurplus that make a plus on the store
	And I close all client application windows
	And Delay 80
	* Preparation
		* Remove control (store 05)
...
Scenario:_8913014 try unpost OpeningEntry that make a plus on the store
	And I close all client application windows
	* Preparation
		* Remove control (store 05)
			When Remove stock control for store 05
