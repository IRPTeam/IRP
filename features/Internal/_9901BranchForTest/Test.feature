...
@blocking
@Items
Сценарий: _1229 Заполнение справоника "Items"
	* Открытие формы и заполнение реквизитов Description_en и ItemID 
		Дано Я открываю основную форму справочника "Items" 
		И в поле с именем 'Description_en' я ввожу текст 'My Item'
		И я перехожу к следующему реквизиту
		И значение поля с именем 'Description_en' содержит текст 'My Item'
		И в поле с именем 'ItemID' я ввожу текст '123'
		И я перехожу к следующему реквизиту
		И значение поля  с именем 'ItemID' содержит текст '123'

	* Заполенение реквизита ItemType
		И я нажимаю кнопку выбора у поля с именем "ItemType"
		И я жду открытия формы "Item types" в течение 10 секунд
		Если в таблице "List" есть колонка "Код" Тогда
			...
			И в таблице "List" я перехожу к строке
				| 'Код' |
				| '1'   |
		ИначеЕсли в таблице "List" есть колонка "Ссылка" Тогда
			И таблица "List" содержит строки:
				| 'Ссылка'                     |
			...
			И в таблице "List" я перехожу к строке
				| 'Ссылка'                     |
				| 'Товар (есть размер и цвет)' |
		ИначеЕсли в таблице "List" есть колонка "Description" Тогда
			И таблица "List" содержит строки:
				| 'Description'                |
				| 'Товар (есть размер и цвет)' |
			И в таблице "List" я перехожу к строке
		...
		И значение поля  с именем  'ItemType' содержит текст 'Товар (есть размер и цвет)'

	* Заполнение реквизита Unit
		И я нажимаю кнопку выбора у поля с именем "Unit"
		И я жду открытия формы "Item units" в течение 10 секунд
		Если в таблице "List" есть колонка "Код" Тогда
			И таблица "List" содержит строки:
				| 'Код' |
				| '1'   |
		...
		ИначеЕсли в таблице "List" есть колонка "Description" Тогда
			...
			И в таблице "List" я перехожу к строке
				...
				| 'шт'          |
		И в таблице "List" я выбираю текущую строку
		И значение поля  с именем 'Unit' содержит текст 'шт'

	* Заполнение реквизита Patners
		И я нажимаю кнопку выбора у поля с именем "Vendor"
		И я жду открытия формы "Partners" в течение 10 секунд
		...
		ИначеЕсли в таблице "List" есть колонка "Description" Тогда
			...
			И в таблице "List" я перехожу к строке
				...
				| 'Поставщик 1 (1 соглашение)' |
		И в таблице "List" я выбираю текущую строку
		И значение поля  с именем 'Vendor' содержит текст 'Поставщик 1 (1 соглашение)'
