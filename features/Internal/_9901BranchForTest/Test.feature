...
Сценарий: Подготовка данных
	...
	И я проверяю или создаю для справочника "SerialLotNumbers" объекты:
		...
		| 'e1cib/data/Catalog.SerialLotNumbers?ref=856b0c9d920fe73211ec8724238260de' | 'False'        | 3      | 'Тестовый номер' | 'e1cib/data/Catalog.Items?ref=b762b13668d0905011eb76684b9f687d' | 'False'    |
...
* Добавление штрихкода
	...
	И в поле с именем 'Barcode' я ввожу текст '111'
	...
	Тогда открылось окно 'Item barcode (create) *'
	И в поле с именем 'Presentation' я ввожу текст '111'
	И я нажимаю на кнопку с именем 'FormWriteAndClose'
	И я жду закрытия окна 'Item barcode (create) *' в течение 10 секунд
		
* Проверка добавленного
	И таблица "List" содержит строки:
		| 'Barcode' | 'Item serial/lot number' | 'Unit' |
		| '111'     | 'Тестовый номер'         | 'шт'   |
...
* Проверка создания копии
	...
	И в поле с именем 'Presentation' я ввожу текст '111'
	И я нажимаю на кнопку с именем 'FormWriteAndClose'
	И у элемента с именем 'ErrorInfo' я жду значения 'Record with these key fields exists! : Item barcode: 111 (Information register: Item barcodes)' в течение 10 секунд		
	И я нажимаю на кнопку с именем 'OK'
	Тогда открылось окно 'Item barcode (create) *'
	И Я закрываю окно 'Item barcode (create) *'
