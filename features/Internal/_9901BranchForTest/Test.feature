...
Сценарий: Подготовка данных
	...
	И я проверяю или создаю для справочника "Units" объекты:
		...
		| 'e1cib/data/Catalog.Units?ref=b762b13668d0905011eb76684b9f687b' | 'False'        | 3      | ''     | 1          | ''          | ''    | 'шт'             | ''                 | ''               | ''               |          |          |          |          |         |
	...
	//#1230
	Сценарий: Проверяю  добавление штрих-кода (Barcodes) 
	...
	Если таблица "List" стала равной: Тогда
		| 'Barcode'       |
		| '2103280010900' |
	Иначе
		И я нажимаю на кнопку с именем 'FormCreate'
		Когда открылось окно 'Item barcode (создание)'
		И в поле с именем 'Barcode' я ввожу текст '2103280010900'
		И я нажимаю на кнопку с именем 'FormWriteAndClose'
		Если таблица "List" стала равной: Тогда
			| 'Barcode'       |
			| '2103280010900' |
		Иначе
			Тогда я вызываю исключение "Не удалось добавить штрихкод"