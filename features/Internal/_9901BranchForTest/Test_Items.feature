...
Сценарий: Подготовка данных
	...
	И я проверяю или создаю для справочника "ItemTypes" объекты:
		...
		| 'e1cib/data/Catalog.ItemTypes?ref=a3a498eecbeac87211ecd518d5bf2264' | 'False'        | ''       | 'False'    | 4      | 'Enum.ItemTypes.Product' | 'False'              | 'Товар без характеристик'    | ''                 | ''               | ''               | '_d446aeb6289741668724e7ec0574e621' |
		| 'e1cib/data/Catalog.ItemTypes?ref=a3a498eecbeac87211ecd518d5bf2263' | 'False'        | ''       | 'False'    | 3      | 'Enum.ItemTypes.Service' | 'False'              | 'Услуга без характеристик'   | ''                 | ''               | ''               | '_b7a0b3a801c4412ca379ee506eca5179' |
		| 'e1cib/data/Catalog.ItemTypes?ref=a3a498eecbeac87211ecd518d5bf2262' | 'False'        | ''       | 'False'    | 2      | 'Enum.ItemTypes.Service' | 'False'              | 'Услуга (есть размер)'       | ''                 | ''               | ''               | '_52497bbe463341d3935d6ebbd1b561f6' |
	...
	И я перезаполняю для объекта табличную часть "AvailableAttributes":
		...
		| 'e1cib/data/Catalog.ItemTypes?ref=a3a498eecbeac87211ecd518d5bf2262' | 'e1cib/data/ChartOfCharacteristicTypes.AddAttributeAndProperty?ref=b762b13668d0905011eb76684b9f6872' | 'False'         | 'False'    | 'False'      |
...
Сценарий: Создание Номенклатуры (услуга без характеристик)

    И я закрываю все окна клиентского приложения
	И Я открываю основную форму списка справочника "Items"
	И я нажимаю на кнопку с именем 'FormCreate'
* Наименование
	И в поле с именем 'Description_en' я ввожу текст 'Услуга без характеристик'
* Вид Номенклатуры
    И из выпадающего списка с именем "ItemType" я выбираю по строке 'Услуга без характеристик'
* Ед.изм
...
*Записать и закрыть
	...
	И я жду закрытия окна 'Item (создание) *' в течение 20 секунд
	
Сценарий: Создание Номенклатуры (услуга с характеристиками)

    И я закрываю все окна клиентского приложения
	И Я открываю основную форму списка справочника "Items"
	И я нажимаю на кнопку с именем 'FormCreate'
* Наименование
	И в поле с именем 'Description_en' я ввожу текст 'Услуга (есть размер)'
* Вид Номенклатуры
    И из выпадающего списка с именем "ItemType" я выбираю по строке 'Услуга (есть размер)'
* Ед.изм
	И я нажимаю кнопку выбора у поля с именем "Unit"
	И в таблице "List" я перехожу к строке:
		| 'Description' |
...
Сценарий: Создание Номенклатуры (товар без характеристик)
...
Сценарий: Создание Номенклатуры (товар с характеристиками)

    И я закрываю все окна клиентского приложения
	И Я открываю основную форму списка справочника "Items"
	И я нажимаю на кнопку с именем 'FormCreate'
* Наименование
	И в поле с именем 'Description_en' я ввожу текст 'Товар (есть размер и цвет)'
* Вид Номенклатуры
