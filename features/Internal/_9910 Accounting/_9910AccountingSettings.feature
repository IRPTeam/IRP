...
Scenario: _0991078 check Bank receipt accounting movements (Return from vendor)
	...
	* Check JE
		...
		And "RegisterRecords" table contains lines
			| 'Period'              | 'Account Dr' | '#' | 'Amount' | 'DebitQuantity' | 'Activity' | 'Credit currency' | 'Ext. Dim. Debit'     | 'Debit amount' | 'Extra dimension2 Dr'                                    | 'Credit quantity' | 'Extra dimension3 Dr' | 'Debit currency' | 'Account Cr' | 'Ext. Dim. Credit'    | 'Operation'                                                                                   | 'Extra dimension2 Cr'                                    | 'Credit amount' | 'Extra dimension3 Cr' |
			| '05.12.2023 10:00:00' | '4020.2'     | '1' | '50,00'  | ''              | 'Yes'      | 'TRY'             | 'Customer and vendor' | '50'           | 'Partner term with vendor (advance payment by document)' | ''                | ''                    | 'TRY'            | '5201'       | 'Customer and vendor' | 'BankReceipt DR (R1020B_AdvancesToVendors) CR (R1021B_VendorsTransactions)'                   | 'Partner term with vendor (advance payment by document)' | '50'            | ''                    |
			| '05.12.2023 10:00:00' | '3250'       | '2' | '50,00'  | ''              | 'Yes'      | 'TRY'             | 'Bank account, TRY'   | '50'           | 'Client and vendor'                                      | ''                | ''                    | 'TRY'            | '4020.2'     | 'Customer and vendor' | 'BankReceipt DR (R3010B_CashOnHand) CR (R1020B_AdvancesToVendors_R1021B_VendorsTransactions)' | 'Partner term with vendor (advance payment by document)' | '50'            | ''                    |
		Then the number of "RegisterRecords" table lines is "равно" "2"
	And I close all client application windows
