...
Scenario: Opening form Catalogs "Incoming messages (IAS)" (гкс_ВходящиеСообщенияRMQ)

	Given I open "гкс_ВходящиеСообщенияRMQ" reference main form
	If the warning is displayed then
		Then I raise "The main form could not be opened Catalogs гкс_ВходящиеСообщенияRMQ" exception
	If current form name is "ErrorWindow" Then
		Then I raise "The main form could not be opened Catalogs гкс_ВходящиеСообщенияRMQ" exception
	And I close current window

Scenario: Opening form Catalogs "Tasks for processing the incoming message queue (IAS)" (гкс_ЗаданияОбработкиОчередиВходящихСообщенийRMQ)

	Given I open "гкс_ЗаданияОбработкиОчередиВходящихСообщенийRMQ" reference main form
	If the warning is displayed then
		Then I raise "The main form could not be opened Catalogs гкс_ЗаданияОбработкиОчередиВходящихСообщенийRMQ" exception
	If current form name is "ErrorWindow" Then
		Then I raise "The main form could not be opened Catalogs гкс_ЗаданияОбработкиОчередиВходящихСообщенийRMQ" exception
	And I close current window

Scenario: Opening form Catalogs "XDTO data models (IAS)" (гкс_МоделиДанныхXDTO)

	Given I open "гкс_МоделиДанныхXDTO" reference main form
	If the warning is displayed then
		Then I raise "The main form could not be opened Catalogs гкс_МоделиДанныхXDTO" exception
	If current form name is "ErrorWindow" Then
		Then I raise "The main form could not be opened Catalogs гкс_МоделиДанныхXDTO" exception
	And I close current window

Scenario: Opening form Catalogs "Queue for deferred formation of outgoing messages (IAS)" (гкс_ОчередьОтложенногоФормированияИсходящихСообщений)

	Given I open "гкс_ОчередьОтложенногоФормированияИсходящихСообщений" reference main form
	If the warning is displayed then
		Then I raise "The main form could not be opened Catalogs гкс_ОчередьОтложенногоФормированияИсходящихСообщений" exception
	If current form name is "ErrorWindow" Then
		Then I raise "The main form could not be opened Catalogs гкс_ОчередьОтложенногоФормированияИсходящихСообщений" exception
	And I close current window

Scenario: Opening form Catalogs "RMQ data recipients (IAS)" (гкс_ПолучателиДанныхRMQ)

	Given I open "гкс_ПолучателиДанныхRMQ" reference main form
	If the warning is displayed then
		Then I raise "The main form could not be opened Catalogs гкс_ПолучателиДанныхRMQ" exception
	If current form name is "ErrorWindow" Then
		Then I raise "The main form could not be opened Catalogs гкс_ПолучателиДанныхRMQ" exception
	And I close current window

Scenario: Opening form Catalogs "RMQ exchange points (IAS)" (гкс_ТочкиОбменаRMQ)

	Given I open "гкс_ТочкиОбменаRMQ" reference main form
	If the warning is displayed then
		Then I raise "The main form could not be opened Catalogs гкс_ТочкиОбменаRMQ" exception
	If current form name is "ErrorWindow" Then
		Then I raise "The main form could not be opened Catalogs гкс_ТочкиОбменаRMQ" exception
	And I close current window

Scenario: Opening form Catalogs "Exchange participants (IAS)" (гкс_УчастникиОбменаRMQ)

	Given I open "гкс_УчастникиОбменаRMQ" reference main form
	If the warning is displayed then
		Then I raise "The main form could not be opened Catalogs гкс_УчастникиОбменаRMQ" exception
	If current form name is "ErrorWindow" Then
		Then I raise "The main form could not be opened Catalogs гкс_УчастникиОбменаRMQ" exception
	And I close current window

Scenario: Opening form Catalogs "Exchange message formats (IAS)" (гкс_ФорматыОбменаИАС)

	Given I open "гкс_ФорматыОбменаИАС" reference main form
	If the warning is displayed then
		Then I raise "The main form could not be opened Catalogs гкс_ФорматыОбменаИАС" exception
	If current form name is "ErrorWindow" Then
		Then I raise "The main form could not be opened Catalogs гкс_ФорматыОбменаИАС" exception
	And I close current window

Scenario: Opening form Catalogs "Message sending tasks (IAS)" (ЗаданияОтправкиИсходящихСообщений)

	Given I open "ЗаданияОтправкиИсходящихСообщений" reference main form
	If the warning is displayed then
		Then I raise "The main form could not be opened Catalogs ЗаданияОтправкиИсходящихСообщений" exception
	If current form name is "ErrorWindow" Then
		Then I raise "The main form could not be opened Catalogs ЗаданияОтправкиИсходящихСообщений" exception
	And I close current window

# Scenario: Opening form Catalogs "Outgoing messages (IAS)" (ИсходящиеСообщения)

# 	Given I open "ИсходящиеСообщения" reference main form
# 	If the warning is displayed then
# 		Then I raise "The main form could not be opened Catalogs ИсходящиеСообщения" exception
# 	If current form name is "ErrorWindow" Then
# 		Then I raise "The main form could not be opened Catalogs ИсходящиеСообщения" exception
# 	And I close current window

Scenario: Opening form Catalogs "Subscriptions to message queues (IAS)" (ПодпискиНаОчередиСообщений)

	Given I open "ПодпискиНаОчередиСообщений" reference main form
	If the warning is displayed then
		Then I raise "The main form could not be opened Catalogs ПодпискиНаОчередиСообщений" exception
	If current form name is "ErrorWindow" Then
		Then I raise "The main form could not be opened Catalogs ПодпискиНаОчередиСообщений" exception
	And I close current window

Scenario: Opening form Catalogs "RMQ queue servers (IAS)" (СерверыОчередейRMQ)

	Given I open "СерверыОчередейRMQ" reference main form
	If the warning is displayed then
		Then I raise "The main form could not be opened Catalogs СерверыОчередейRMQ" exception
	If current form name is "ErrorWindow" Then
		Then I raise "The main form could not be opened Catalogs СерверыОчередейRMQ" exception
	And I close current window

Scenario: Opening form Catalogs "Mock service data" (Unit_MockServiceData)

	Given I open "Unit_MockServiceData" reference main form
	If the warning is displayed then
		Then I raise "The main form could not be opened Catalogs Unit_MockServiceData" exception
	If current form name is "ErrorWindow" Then
		Then I raise "The main form could not be opened Catalogs Unit_MockServiceData" exception
	And I close current window

Scenario: Opening form Catalogs "Service exchange history" (Unit_ServiceExchangeHistory)

	Given I open "Unit_ServiceExchangeHistory" reference main form
	If the warning is displayed then
		Then I raise "The main form could not be opened Catalogs Unit_ServiceExchangeHistory" exception
	If current form name is "ErrorWindow" Then
		Then I raise "The main form could not be opened Catalogs Unit_ServiceExchangeHistory" exception
	And I close current window
