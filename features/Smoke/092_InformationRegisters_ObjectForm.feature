...
Scenario: Opening form Information registers "Identifiers of synchronized objects (IAS)" (гкс_ИдентификаторыСинхронизируемыхОбъектов)

	Given I open "гкс_ИдентификаторыСинхронизируемыхОбъектов" information register default form
	If the warning is displayed then
		Then I raise "The main form could not be opened Information registers гкс_ИдентификаторыСинхронизируемыхОбъектов" exception
	If current form name is "ErrorWindow" Then
		Then I raise "The main form could not be opened Information registers гкс_ИдентификаторыСинхронизируемыхОбъектов" exception
	And I close current window

Scenario: Opening form Information registers "Integration adapter settings (IAS)" (гкс_НастройкиИнтеграционногоАдаптера)

	Given I open "гкс_НастройкиИнтеграционногоАдаптера" information register default form
	If the warning is displayed then
		Then I raise "The main form could not be opened Information registers гкс_НастройкиИнтеграционногоАдаптера" exception
	If current form name is "ErrorWindow" Then
		Then I raise "The main form could not be opened Information registers гкс_НастройкиИнтеграционногоАдаптера" exception
	And I close current window

Scenario: Opening form Information registers "Exchange format settings Enterprise Data (IAS)" (гкс_НастройкиФорматовОбменаED)

	Given I open "гкс_НастройкиФорматовОбменаED" information register default form
	If the warning is displayed then
		Then I raise "The main form could not be opened Information registers гкс_НастройкиФорматовОбменаED" exception
	If current form name is "ErrorWindow" Then
		Then I raise "The main form could not be opened Information registers гкс_НастройкиФорматовОбменаED" exception
	And I close current window

Scenario: Opening form Information registers "Common exchange format settings (IAS)" (гкс_НастройкиФорматовОбменаОбщие)

	Given I open "гкс_НастройкиФорматовОбменаОбщие" information register default form
	If the warning is displayed then
		Then I raise "The main form could not be opened Information registers гкс_НастройкиФорматовОбменаОбщие" exception
	If current form name is "ErrorWindow" Then
		Then I raise "The main form could not be opened Information registers гкс_НастройкиФорматовОбменаОбщие" exception
	And I close current window

Scenario: Opening form Information registers "Deferred object processing (IAS)" (гкс_ОтложеннаяОбработкаОбъектов)

	Given I open "гкс_ОтложеннаяОбработкаОбъектов" information register default form
	If the warning is displayed then
		Then I raise "The main form could not be opened Information registers гкс_ОтложеннаяОбработкаОбъектов" exception
	If current form name is "ErrorWindow" Then
		Then I raise "The main form could not be opened Information registers гкс_ОтложеннаяОбработкаОбъектов" exception
	And I close current window

Scenario: Opening form Information registers "Incoming message processing queue (IAS)" (гкс_ОчередьОбработкиВходящихСообщенийRMQ)

	Given I open "гкс_ОчередьОбработкиВходящихСообщенийRMQ" information register default form
	If the warning is displayed then
		Then I raise "The main form could not be opened Information registers гкс_ОчередьОбработкиВходящихСообщенийRMQ" exception
	If current form name is "ErrorWindow" Then
		Then I raise "The main form could not be opened Information registers гкс_ОчередьОбработкиВходящихСообщенийRMQ" exception
	And I close current window

Scenario: Opening form Information registers "Data exchange issues (IAS)" (гкс_ПроблемыОбменаДанными)

	Given I open "гкс_ПроблемыОбменаДанными" information register default form
	If the warning is displayed then
		Then I raise "The main form could not be opened Information registers гкс_ПроблемыОбменаДанными" exception
	If current form name is "ErrorWindow" Then
		Then I raise "The main form could not be opened Information registers гкс_ПроблемыОбменаДанными" exception
	And I close current window

Scenario: Opening form Information registers "States of outgoing messages (IAS)" (гкс_СостоянияИсходящихСообщений)

	Given I open "гкс_СостоянияИсходящихСообщений" information register default form
	If the warning is displayed then
		Then I raise "The main form could not be opened Information registers гкс_СостоянияИсходящихСообщений" exception
	If current form name is "ErrorWindow" Then
		Then I raise "The main form could not be opened Information registers гкс_СостоянияИсходящихСообщений" exception
	And I close current window
