...
Сценарий:  _0154005 проверка прорисовки доп реквизитов на форме без перезахода (справочник Boxes)
	Тогда я проверяю наличие элемента плана вида характеристик "AddAttributeAndPropertyValues" со значением поля Description Eng "Test"
	И я создаю тестовый элемент справочника Boxes
		И я открываю навигационную ссылку 'e1cib/list/Catalog.Boxes'
		И я нажимаю на кнопку с именем 'FormCreate'
		И в поле 'ENG' я ввожу текст 'Test'
		И я нажимаю кнопку выбора у поля "Item type"
		И в таблице "List" я выбираю текущую строку
	...
	И я открываю форму Partners
		...
		И в таблице "List" я выбираю текущую строку
		И элемент формы "Test" отсутствует на форме
	И по выбранному Box я добавляю доп реквизит Test не закрывая форму
		И я открываю навигационную ссылку 'e1cib/list/Catalog.AddAttributeAndPropertySets'
		И в таблице "List" я перехожу к строке:
			| Predefined data item name     |
			| Catalog_Boxes              |
		...
		И я нажимаю на кнопку 'Save and close'
	Тогда я проверяю отображение доп реквизита на форме
		И Я нажимаю кнопку командного интерфейса 'Boxes'
		И элемент формы "Test" присутствует на форме
	И я закрыл все окна клиентского приложения
...
Сценарий:  _015400682 проверка прорисовки доп реквизитов на форме без перезахода (документ Unboxing)
	Тогда я проверяю наличие элемента плана вида характеристик "AddAttributeAndPropertyValues" со значением поля Description Eng "Test"
	И я открываю форму для создания Unboxing
		И я открываю навигационную ссылку 'e1cib/list/Document.Unboxing'
		И я нажимаю на кнопку с именем 'FormCreate'
		И я перехожу к закладке "Other"
		И элемент формы "Test" отсутствует на форме
	И по Unboxing я добавляю доп реквизит Test не закрывая форму
		И я открываю навигационную ссылку 'e1cib/list/Catalog.AddAttributeAndPropertySets'
		И в таблице "List" я перехожу к строке:
			| Predefined data item name     |
			| Document_Unboxing             |
		...
		И в таблице "List" я выбираю текущую строку
		И в таблице "Attributes" я завершаю редактирование строки
		И в поле 'ENG' я ввожу текст 'Unboxing'
		И я нажимаю на кнопку 'Save and close'
	Тогда я проверяю отображение доп реквизита на форме
		И Я нажимаю кнопку командного интерфейса 'Unboxing (create)'
		И элемент формы "Test" присутствует на форме
	И я закрыл все окна клиентского приложения
...
Сценарий:  _0154007 очистка от элемента Test
	...
	И я удаляю элемент справочника "Boxes" со значением поля Description_en "Test"
...
Сценарий: _0154028 проверяю автозаполнение item key в Boxing по item у которых только один item key
	И я открываю навигационную ссылку 'e1cib/list/Document.Boxing'
	И я нажимаю на кнопку с именем 'FormCreate'
	Когда проверяю автозаполнение item key в документах набора товаров по складу по item у которых только один item key

Сценарий: _0154029 проверяю автозаполнение item key в Unboxing по item у которых только один item key
	И я открываю навигационную ссылку 'e1cib/list/Document.Unboxing'
	И я нажимаю на кнопку с именем 'FormCreate'
	Когда проверяю автозаполнение item key в документах набора товаров по складу по item у которых только один item key
...
Сценарий: _0154063 проверка ввода по строке item и item key в документе Boxing на английском
	* Открытие формы создания Bundling
		И я открываю навигационную ссылку "e1cib/list/Document.Boxing"
		И я нажимаю на кнопку с именем 'FormCreate'
	* Ввод по строке item и item key
...
Сценарий: _0154064 проверка ввода по строке item и item key в документе UnBoxing на английском
	* Открытие формы создания UnBoxing
		И я открываю навигационную ссылку "e1cib/list/Document.Unboxing"
		И я нажимаю на кнопку с именем 'FormCreate'
	* Ввод по строке item и item key
		И в таблице "ItemList" я нажимаю на кнопку с именем 'ItemListAdd'
...
Сценарий: _0154065 проверка ввода по строке item, item key, properties, box в документе Price list на английском
	И я закрыл все окна клиентского приложения
	...
	* Проверка введенных значений
		...
		| 'Item'     | 'Item key'  |
		| 'Trousers' | '36/Yellow' |
	# * Проверка ввода по строке коробки (Box)
	# 	И я меняю значение переключателя 'Set price' на 'By Items'
	# 	И я меняю значение переключателя 'InputTypeItemList' на 'Box'
	# 	И я нажимаю на кнопку с именем 'ItemListAdd'
	# 	И в таблице "ItemList" из выпадающего списка "Item" я выбираю по строке 'paper'
	# 	И     таблица "ItemList" содержит строки:
	# 	| 'Item'      |
	# 	| 'Paper box' |
	# 	И я закрыл все окна клиентского приложения
...
Сценарий: _0154079 проверка ввода по строке company, store, item box в документе Boxing на английском
	* Открытие формы создания Boxing
		И я открываю навигационную ссылку "e1cib/list/Document.Boxing"
		И я нажимаю на кнопку с именем 'FormCreate'
	* Ввод по строке Company
		И из выпадающего списка "Company" я выбираю по строке 'main'
	* Ввод по строке store
		И из выпадающего списка с именем "Store" я выбираю по строке '01'
	* Ввод по строке Item box
		И из выпадающего списка с именем "ItemBox" я выбираю по строке 'Paper'
	* Проверка введенных значений
		И     элемент формы с именем "Company" стал равен 'Main Company'
		И     элемент формы с именем "Store" стал равен 'Store 01'
		И     элемент формы с именем "ItemBox" стал равен 'Paper box'
	И я закрыл все окна клиентского приложения

Сценарий: _0154080 проверка ввода по строке company, store, item box в документе UnBoxing на английском
	* Открытие формы создания Unboxing
		И я открываю навигационную ссылку "e1cib/list/Document.Unboxing"
		И я нажимаю на кнопку с именем 'FormCreate'
	* Ввод по строке Company
