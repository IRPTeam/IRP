...
Сценарий: сохранении информации по Item без заполненного item key в документе Boxing
	* Открытие формы документа Boxing
...
Сценарий: сохранении информации по Item без заполненного item key в документе Unboxing
	И Я закрыл все окна клиентского приложения
	* Открытие формы документа Unboxing
	...
	* Добавление Item без item key
		И я заполняю основные реквизиты документа
			...
			И в таблице "List" я выбираю текущую строку
		И я нажимаю на кнопку с именем "ItemListContextMenuAdd"
		И в таблице "ItemList" я нажимаю кнопку выбора у реквизита "Item"
		И в таблице "List" я перехожу к строке:
			| 'Description'   |
			| 'Dress'         |
