...
Сценарий: _200001 заполнение пользовательских настроек по пользователю CI
	...
	* Заполнение пользовательских настроек для Bank receipt
		...
		И в таблице "List" я перехожу к строке:
			...
			| 'USD'  | 'American dollar' |
		И в таблице "List" я выбираю текущую строку
	* Заполнение пользовательских настроек для Boxing
		И в таблице "MetadataTree" я перехожу к строке:
			| 'Group name' |
			| 'Boxing'     |
		И в таблице "MetadataTree" я активизирую поле "Group name"
	...
	* Заполнение пользовательских настроек в Shipment confirmation
		...
		И в таблице "List" я перехожу к строке:
			...
			| 'Store 02'    |
		И в таблице "List" я выбираю текущую строку
	* Заполнение пользовательских настроек в Unboxing
		И в таблице "MetadataTree" я перехожу к строке:
		| 'Group name' |
		| 'Unboxing'   |
		И в таблице "MetadataTree" я перехожу к строке:
		...
		И в таблице "MetadataTree" я перехожу к строке:
			...
			| 'Write off'  | 'No'  |
		И в таблице "MetadataTree" я выбираю текущую строку
		И в таблице "MetadataTree" из выпадающего списка "Value" я выбираю точное значение 'Yes'
...
Сценарий:  _200006 проверка заполнения документа Boxing из пользовательских настроек
	И я открываю навигационную ссылку 'e1cib/list/Document.Boxing'
	И я нажимаю на кнопку с именем 'FormCreate'
	Тогда я проверяю заполнение полей из пользовательских настроек
		И     элемент формы с именем "Company" стал равен 'Main Company'
		И     элемент формы с именем "Store" стал равен 'Store 03'
		И     элемент формы с именем "Receipt" стал равен 'No'
		И     элемент формы с именем "WriteOff" стал равен 'No'
	И я закрыл все окна клиентского приложения
...
Сценарий:  _200025 проверка заполнения документа Unboxing из пользовательских настроек
	И я открываю навигационную ссылку 'e1cib/list/Document.Unboxing'
	И я нажимаю на кнопку с именем 'FormCreate'
	Тогда я проверяю заполнение полей из пользовательских настроек
		И     элемент формы с именем "WriteOff" стал равен 'Yes'
	И я закрыл все окна клиентского приложения
