...
Сценарий: проверка заполнение склада в документе Sales order
	...
	* Добавление товаров
		...
		И в таблице "ItemList" я завершаю редактирование строки
	* Проверка заполнения формы Sales order
	...
	* Очистка значения склада
		...
		И я нажимаю на кнопку 'OK'
	* Проверка заполнения склада из соглашения
		И     таблица "ItemList" содержит строки:
			| 'Item'  | 'Item key' | 'Q'     | 'Store'    |
			| 'Dress' | 'XS/Blue'  | '2,000' | 'Store 01' |
			| 'Shirt' | '36/Red'   | '1,000' | 'Store 01' |
...
Сценарий: проверка заполнение склада в документе Purchase order
	* Открытие формы документа Purchase order
	...
	* Добавление товаров
		...
		И в таблице "ItemList" в поле 'Q' я ввожу текст '1,000'
		И в таблице "ItemList" я завершаю редактирование строки
	* Проверка заполнения формы Purchase order
		И     элемент формы с именем "Partner" стал равен 'Ferron BP'
		И     элемент формы с именем "LegalName" стал равен 'Company Ferron BP'
	...
	* Очистка значения склада
		И в поле с именем 'Store' я ввожу текст ''
		И я нажимаю на кнопку 'OK'
	* Проверка заполнения склада из пользовательских настроек (склад в соглашении не указан)
		И     таблица "ItemList" содержит строки:
			| 'Item'  | 'Item key' | 'Q'     | 'Store'    |
			| 'Dress' | 'XS/Blue'  | '2,000' | 'Store 03' |
			| 'Shirt' | '36/Red'   | '1,000' | 'Store 03' |
	* Перевыбор соглашения с пустым складом и проверка заполнения склада из пользовательских настроек
		И я нажимаю кнопку выбора у поля "Agreement"
		И в таблице "List" я перехожу к строке:
			| 'Description' |
			| 'Vendor Ferron, TRY' |
...
Сценарий: проверка заполнение склада в документе Purchase invoice
	* Открытие формы документа Purchase invoice
...
Сценарий: _0154516 проверка заполнения склада в Shipment confirmation
	...
	* Перевичное заполнение склада и товаров
		...
		* Добавление второй строки
			...
			| 'Store 02'    |
			И в таблице "List" я выбираю текущую строку
		* Проверка заполнения склада по строкам
			И     таблица "ItemList" содержит строки:
...
Сценарий: _0154517 проверка заполнения склада в Goods receipt
