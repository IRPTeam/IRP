...
Функционал: проверка заполнения и перезаполнения форм документов + подключение формы по валютам
...
Сценарий: _0154100 создание тестовых данных для тестирования заполнения и перезаполнения документов
	...
	* Для теста по выбору Planing transaction basis в банковских/кассовых документах
		...
		* Создание Cashtransfer order на обмен валюты между кассами
			...
			И я заполняю информацию о получателе и получаемой сумме
				...
				И в таблице "List" я перехожу к строке:
					...
					| 'Arina Brown' |
				И в таблице "List" я выбираю текущую строку
			И я изменяю номер документа на 11
		...
		* Создание Cashtransfer order на обмен валюты между банковскими счетами
			...
			И я изменяю номер документа на 13
		...
		* Создание Cashtransfer order на перемещение между банковскими счетами в одной валюте
			...
			* Создание Cashtransfer order на перемещение между банковскими счетами в одной валюте
				...
				И я изменяю номер документа на 14
...
Сценарий: _0154101 проверка заполнения и перезаполнения Sales order
	И Я закрыл все окна клиентского приложения
	...
	* Проверка заполнения цены по новым строкам при перевыборе соглашения
		* Добавление строки
			И в таблице "ItemList" я нажимаю на кнопку с именем 'ItemListAdd'
			И в таблице "ItemList" я нажимаю кнопку выбора у реквизита с именем "ItemListItem"
			И в таблице "List" я перехожу к строке:
	...
	* Проверка расчета налогов при заполнении компании при перевыборе соглашения
		* Перевыбор соглашения
			И я нажимаю кнопку выбора у поля "Agreement"
			И в таблице "List" я перехожу к строке:
				| 'Description'           |
	...
	* Проверка заполнения цены и расчет налогов при добавлении товара через поиск штрих-кодов
		* Добавление товара через штрих-кодов
			...
			И я нажимаю на кнопку 'OK'
			И Пауза 4
		* Проверка заполнения цены и расчета налогов
			И     таблица "ItemList" содержит строки:
				| 'Price'  | 'Item'     | 'VAT' | 'Item key'  | 'Procurement method' | 'Tax amount' | 'SalesTax' | 'Q'     | 'Unit' | 'Net amount' | 'Total amount' | 'Store'    |
				| '400,00' | 'Trousers' | '18%' | '38/Yellow' | 'Stock'              | '64,98'      | '1%'       | '1,000' | 'pcs'  | '335,02'     | '400,00'       | 'Store 01' |
				| '350,00' | 'Shirt'    | '18%' | '38/Black'  | 'Stock'              | '113,71'     | '1%'       | '2,000' | 'pcs'  | '586,29'     | '700,00'       | 'Store 01' |
			...
			И Пауза 4
	* Проверка заполнения цены и расчет налогов при добавлении товара через форму подбора товаров
	...
	* Проверка перерачета налогов при снятии/повторной установке галочки Price include Tax
		...
		* Установка галочки Price include Tax и проверка расчета
			...
			И     таблица "ItemList" содержит строки:
				...
				| '550,00' | 'Dress' | 'L/Green'  | '89,35'      | '1%'       | '1,000' | 'pcs'  | '460,65'     | '550,00'       | 'Store 01' |
				| '520,00' | 'Dress' | 'XS/Blue'  | '84,47'      | '1%'       | '1,000' | 'pcs'  | '435,53'     | '520,00'       | 'Store 01' |
	* Проверка заполнения галочки Price include Tax при перевыборе соглашения и проверка пересчета налогов
		...
		* Проверка перерасчета налогов
			И     таблица "ItemList" содержит строки:
				...
				| '550,00' | 'Dress' | 'L/Green'  | '89,35'      | '1%'       | '1,000' | 'pcs'  | '460,65'     | '550,00'       | 'Store 01' |
				| '520,00' | 'Dress' | 'XS/Blue'  | '84,47'      | '1%'       | '1,000' | 'pcs'  | '435,53'     | '520,00'       | 'Store 01' |
		* Проверка заполнения таблицы валют
...
Сценарий: _0154102 проверка заполнения и перезаполнения Sales invoice
...
Сценарий: _0154105 проверка заполнения и перезаполнения Purchase order
...
Сценарий: _0154106 проверка заполнения и перезаполнения Purchase invoice
...
Сценарий: _0154107 проверка заполнения и перезаполнения Cash reciept (вид операции Payment from customer)
	...
	* Проверка заполнения компании
		И я нажимаю кнопку выбора у поля "Company"
		И в таблице "List" я перехожу к строке:
			| Description  |
			| Main Company |
		И в таблице "List" я выбираю текущую строку
	* Проверка заполнения валюты до выбора кассы
		И я нажимаю кнопку выбора у поля "Currency"
		И в таблице "List" я перехожу к строке:
			| Code |
			| USD  |
		И в таблице "List" я выбираю текущую строку
	* Проверка заполнения кассы (мультивалютная)
	...
	* Проверка заполнения Agreement при добавлении партнера если оно у партнера одно
		И в таблице "PaymentList" я нажимаю на кнопку с именем 'PaymentListAdd'
...
Сценарий: _0154109 проверка заполнения и перезаполнения Bank reciept (вид операции Payment from customer)
	...
	* Проверка заполнения компании
		...
		И в таблице "List" я перехожу к строке:
			...
			| Main Company |
		И в таблице "List" я выбираю текущую строку
	* Проверка заполнения валюты до выбора счета
		И я нажимаю кнопку выбора у поля "Currency"
		И в таблице "List" я перехожу к строке:
			| Code |
			| USD  |
...
Сценарий: _0154111 проверка заполнения и перезаполнения Cash payment (вид операции Payment to the vendor)
...
Сценарий: _0154113 проверка заполнения и перезаполнения Bank payment (вид операции Payment to the vendor)
	...
	* Проверка заполнения компании
		...
		И в таблице "List" я перехожу к строке:
			...
			| Main Company |
		И в таблице "List" я выбираю текущую строку
	* Проверка заполнения валюты до выбора банковского счета
		И я нажимаю кнопку выбора у поля "Currency"
		И в таблице "List" я перехожу к строке:
			| Code |
			| USD  |
...
Сценарий: _0154115 проверка заполнения и перезаполнения Cash transfer order
	* Открытие формы Cash transfer order
		...
		И в таблице "List" я перехожу к строке:
			...
			| 'Main Company' |
		И в таблице "List" я выбираю текущую строку
	* Проверка заполнения валюты пи выборе банка/кассы если она зафиксированна
		И я нажимаю кнопку выбора у поля "Sender"
		И в таблице "List" я перехожу к строке:
	...
	* Проверка заполнения суммы в Receive amount из Send amount в случае совпадения валют
		И я нажимаю кнопку выбора у поля "Sender"
		И в таблице "List" я перехожу к строке:
			| 'Description'  |
			| 'Cash desk №2' |
	...
	* Проверка заполнения Send date и Receive date
...
Сценарий: _0154116 проверка заполнения и перезаполнения Cash expence
...
Сценарий: _0154117 проверка заполнения и перезаполнения Cash revenue
...
Сценарий: _0154118 проверка очистки реквизитов на форме Cash reciept при перевыборе вида операции
	* Открытие формы CashReceipt
		И я открываю навигационную ссылку 'e1cib/list/Document.CashReceipt'
		И я нажимаю на кнопку с именем 'FormCreate'
	* Заполнение реквизитов шапки CashReceipt
		И я нажимаю кнопку выбора у поля "Company"
		И в таблице "List" я перехожу к строке:
			| Description  |
			| Main Company |
...
Сценарий: _0154119 проверка очистки реквизитов на форме Cash payment при перевыборе вида операции
	* Открытие формы CashPayment
		И я открываю навигационную ссылку 'e1cib/list/Document.CashPayment'
		И я нажимаю на кнопку с именем 'FormCreate'
	* Заполнение реквизитов шапки CashPayment
		И я нажимаю кнопку выбора у поля "Company"
		И в таблице "List" я перехожу к строке:
			| Description  |
			| Main Company |
...
Сценарий: _0154120 проверка очистки реквизитов на форме Bank reciept при перевыборе вида операции
	...
	* Проверка очистки полей 'Agreement' и 'Payer' при перевыборе вида операции на Currency exchange
		...
		| '1' | ''       | ''                | ''                          |
		* Проверка заполнения Transit account из Accountant
...
Сценарий: _0154121 проверка очистки реквизитов на форме Bank payment при перевыборе вида операции
	* Открытие формы BankPayment
		И я открываю навигационную ссылку 'e1cib/list/Document.BankPayment'
		И я нажимаю на кнопку с именем 'FormCreate'
	* Заполнение реквизитов шапки BankPayment
		И я нажимаю кнопку выбора у поля "Company"
		И в таблице "List" я перехожу к строке:
			| Description  |
			| Main Company |
...
Сценарий: _0154122 проверка заполнения и перезаполнения Reconcilation statement
...
Сценарий: _0154123 заполнение Transit account из Account при обмене валюты в BankReceipt
	...
	* Открытие формы BankReceipt и выбор типа операции Currency exchange
		...
		И я нажимаю на кнопку с именем 'FormCreate'
		И из выпадающего списка "Transaction type" я выбираю точное значение 'Currency exchange'
	* Проверка заполнения Transit account 
		И я нажимаю кнопку выбора у поля "Account"
		И в таблице "List" я перехожу к строке:
			| 'Description'       |
			| 'Bank account, USD' |
	...
	* Проверка заполнения Transit account при перевыборе Bank account
		И я нажимаю кнопку выбора у поля "Account"
		И в таблице "List" я перехожу к строке:
			| 'Currency' | 'Description'       |
			| 'TRY'      | 'Bank account, TRY' |
...
Сценарий: _053014 проверка отображения реквизитов на форме BankPayment с видом операции Currency exchange
	И Я закрыл все окна клиентского приложения
	И я открываю навигационную ссылку 'e1cib/list/Document.BankPayment'
	И я нажимаю на кнопку с именем 'FormCreate'
	И из выпадающего списка "Transaction type" я выбираю точное значение 'Currency exchange'
...
Сценарий: проверка заполнения и перезаполнения документа CreditDebitNote
	* Создание документа
