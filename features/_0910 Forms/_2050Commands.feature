...
Сценарий: _0205015 добавление тестовой команды в список документов Boxing
	* Открытие регистра команд
		И я открываю навигационную ссылку 'e1cib/list/InformationRegister.ExternalCommands'
		И я нажимаю на кнопку с именем 'FormCreate'
	* Заполнение данных для тестовой команды для Boxing
		* Создание метаданного для Boxing и его выбор для команды
			И я нажимаю кнопку выбора у поля "Configuration metadata"
			И в таблице "List" я перехожу к строке:
				| 'Description' |
				| 'Documents'   |
			И в таблице  "List" я перехожу на один уровень вниз
			И я нажимаю на кнопку с именем 'FormCreate'
			И в поле 'Description' я ввожу текст 'Boxing'
			И я нажимаю кнопку выбора у поля "Parent"
			И в таблице "List" я перехожу к строке:
				| 'Description' |
				| 'Documents'   |
	...
	* Проверка сохранения команды
		...
		| 'Configuration metadata'       | 'External data proc' |
		| 'Boxing'                | 'Test command'       |
	* Проверка вызова команды из списка документов Boxing
...
Сценарий: _0205036 добавление тестовой команды в список документов Unboxing
	* Открытие регистра команд
		И я открываю навигационную ссылку 'e1cib/list/InformationRegister.ExternalCommands'
		И я нажимаю на кнопку с именем 'FormCreate'
	* Заполнение данных для тестовой команды для Unboxing
		* Создание метаданного для Unboxing и его выбор для команды
			И я нажимаю кнопку выбора у поля "Configuration metadata"
			И в таблице "List" я перехожу к строке:
				| 'Description' |
				| 'Documents'   |
			И в таблице  "List" я перехожу на один уровень вниз
			И я нажимаю на кнопку с именем 'FormCreate'
			И в поле 'Description' я ввожу текст 'Unboxing'
			И я нажимаю кнопку выбора у поля "Parent"
			И в таблице "List" я перехожу к строке:
				| 'Description' |
				| 'Documents'   |
	...
	* Проверка сохранения команды
		...
		Тогда таблица "List" содержит строки:
		| 'Configuration metadata'       | 'External data proc' |
		| 'Unboxing'                | 'Test command'       |
	* Проверка вызова команды из списка документов Unboxing
