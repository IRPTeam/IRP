...
Сценарий: _350000 создание тестовых данных для проверки связки  доп реквизитов в item type и их отображении в наборе для item key/price key
...
Сценарий: _350006 проверка ошибки при задваивании доп реквизита по item 
    ...
    * Проверка сообщения об ошибке сохранения
        И я нажимаю на кнопку 'Save'
        Тогда я жду, что в сообщениях пользователю будет подстрока "Dublicated attribute: Brand TR" в течение 10 секунд
    И я закрыл все окна клиентского приложения
...
Сценарий: _350007 проверка ошибки при задваивании доп реквизита составляющего item key
    ...
    * Проверка сообщения об ошибке сохранения
        И я нажимаю на кнопку 'Save'
        Тогда я жду, что в сообщениях пользователю будет подстрока "Dublicated attribute: Brand Socks TR" в течение 10 секунд
    И я закрыл все окна клиентского приложения